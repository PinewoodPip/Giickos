...
public class Observable<T extends Enum<T>> extends GiickosFragment {
    private HashMap<T, List<edu.ub.pis.giickos.model.observer.Observer>> observers;

    public Observable() {
        this.observers = new HashMap<>();
    }

    public void subscribe(T eventType, edu.ub.pis.giickos.model.observer.Observer observer) {
    ...
    public void unsubscribe(T eventType, edu.ub.pis.giickos.model.observer.Observer observer) {
        getObservers(eventType).remove(observer);
    }

    protected void notifyObservers(T eventType, ObservableEvent eventData) {
        for (edu.ub.pis.giickos.model.observer.Observer observer : getObservers(eventType)) {
            observer.update(eventData);
        }
    }

    private List<edu.ub.pis.giickos.model.observer.Observer> getObservers(T eventType) {
